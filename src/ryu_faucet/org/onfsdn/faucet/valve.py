...
class Valve(object):
    ...
    def __init__(self, dp, logname, *args, **kwargs):
        ...
        self.ddosWatcher = DDOSWatcher(self.logger)
        #self.packet_watcher.repeat()
    ...
    def _rate_limit_packet_ins(self):
        ...
        print('Entered rate limit function')
        ...
        if self.dp.ignore_learn_ins:
            if self._packet_in_count_sec % self.dp.ignore_learn_ins == 0:
                ...
                return True
        self.logger.debug('NOT RATE LIMITED')
        return False



    #PLACE HOLDER    
    def _rate_limit_packet_ins_source(self, pkt):
        
        self.logger.debug('ENTERED NEW CUSTOM RATE LIMIT FUNCTION')
...
class DDOSWatcher:
    ...
    # Packet sampling time window
    packet_receive_window = 1
    # Maximum number of packets allow per time window
    packet_receive_max_count = 3
    
    # TESTING: Random MAC addresses:
    randomMACs = ['00:01', '00:02', '00:03', '00:04', '00:05']
    
    def __init__(self, logger):
        print('New PacketWatcher')
        self.logger = logger
    ...
    def limit_pkt_src(self, pkt):
        ...
        if number_recent_occurence > self.packet_receive_max_count:
            ...
            self.logger.debug('\nDDOS ALERT: too many packets received from ' + pkt[0] + '. Packet dropped')
            return True
    ...
    def pkt_num_recent_occurence(self, pkt_to_find):
        count = 0
        for stored_pkt in self.recent_packets:
            # Stored packet's source matches this packet's source
            if stored_pkt[0] == pkt_to_find:
                # Stored packet is within sampling time window
